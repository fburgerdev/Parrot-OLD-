...
namespace Parrot
{
	namespace Math
	{
		...
		public:
			...
			inline bool operator>=(const Vector2<T>& other) { return x >= other.x && y >= other.y; }
		private:
			friend std::ostream& operator<<(std::ostream& stream, const Vector2<T>& vector) { stream << "Vec2(" << vector.x << ", " << vector.y << ")"; return stream; }
		};
		...
		public:
			...
			inline bool operator>=(const Vector3<T>& other) { return x >= other.x && y >= other.y && z >= other.z; }
		private:
			friend std::ostream& operator<<(std::ostream& stream, const Vector3<T>& vector) { stream << "Vec3(" << vector.x << ", " << vector.y << ", " << vector.z << ")"; return stream; }
		};
