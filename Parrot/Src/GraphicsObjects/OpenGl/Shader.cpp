#include "Ptpch.hpp"
#ifdef PT_OPENGL
    #include "GraphicsObjects/Shader.hpp"
    #include "GlDebug.hpp"
    ...
    namespace Parrot
    {
        namespace GraphicsObject
        {
            ...
            {
                PT_ASSERT(m_ID != 0, "Could not bind shader!");
            ...
            uint32_t Shader::CompileShader(uint32_t type, const std::string& source)
            {
                ...
                if (result == GL_FALSE)
                {
                    ...
                    if (type == GL_VERTEX_SHADER)
                        Log::LogError("Failed to compile vertex shader \"%\"!\n%\n%Source Code:", FilenameFromPath(m_Filepath.c_str()), message, Log::White);
                    else
                        Log::LogError("Failed to compile fragment shader \"%\"!\n%\n%Source Code:", FilenameFromPath(m_Filepath.c_str()), message, Log::White);
                    ...
                    uint32_t line = 2;
                    Log::ContinuePreviousLog();
                    std::cout << Log::White << "1\t";
                    while (*sourceCursor != '\0')
                    {
                        ...
                        if (*sourceCursor++ == '\n' && *sourceCursor != '\0')
                        {
                            ...
                            for (uint32_t i = 0; i < errorLines.size(); i++)
                            {
                                ...
                                {
                                    std::cout << Log::Red << line++ << '\t';
                                    goto LoopExit2;
                                }
                            }
                            std::cout << Log::White << line++ << '\t';
                        }
                    }
                ...
                if (type == GL_VERTEX_SHADER)
                    Log::LogInfo("Vertex Shader \"%\" compiled successfully!", FilenameFromPath(m_Filepath.c_str()));
                else 
                    Log::LogInfo("Fragment Shader \"%\" compiled successfully!", FilenameFromPath(m_Filepath.c_str()));
                return id;
            }
