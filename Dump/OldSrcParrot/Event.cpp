...
namespace Parrot
{
	...
	static constexpr const char* s_MouseButtonAction[2] = { "Release", "Press" };
	static constexpr const char* s_KeyboardAction[3] = { "Release", "Press", "Repeat" };
	...
	void Parrot::Event::Log() const
	{
		...
		case Type::WindowResizeEvent:
			Log::LogEvent("WindowResizeEvent: width = %u, height = %u", windowResizeEvent.dim.x, windowResizeEvent.dim.y);
			break;
		case Type::WindowMaximizeEvent:
			Log::LogEvent("WindowMaximizeEvent:");
			break;
		case Type::MouseMoveEvent:
			Log::LogEvent("MouseMoveEvent: x = %u, y = %u", mouseMoveEvent.pos.x, mouseMoveEvent.pos.y);
			break;
		case Type::MouseButtonEvent:
			Log::LogEvent("MouseButtonEvent: button = %s, action = %s", s_MouseButtonStrings[(uint8_t)mouseButtonEvent.button], s_MouseButtonAction[(uint8_t)mouseButtonEvent.action]);
			break;
		case Type::MouseScrollEvent:
			Log::LogEvent("MouseScrollEvent: xOffset = %i, yOffset = %i", mouseScrollEvent.xOffset, mouseScrollEvent.yOffset);
			break;
		case Type::KeyboardEvent:
			Log::LogEvent("KeyboardEvent: key = %s, action = %s", s_KeyStrings[(uint16_t)keyboardEvent.key], s_KeyboardAction[(uint8_t)keyboardEvent.action]);
			break;
		default:
			break;
		}
